...
import { fakeSelectedFrames } from "../../fakedata";

interface DivergenceTreeProps {
  selectedFrames: D3StackFrame[];
}

const DivergenceTree: React.FC<DivergenceTreeProps> = React.memo(
  ({ selectedFrames = fakeSelectedFrames }) => {
    ...
    const getFrames = (
      ...
      if (
        !flowDiv ||
        !flowDiv.originalPosition.stop ||
        !flowDiv.modifiedPosition.stop
      ) {
        return {
    ...
    const buildRows = (): TreeRow[] => {
      ...
      while (
        ...
        rows.push({
          ...
          isOriginalSelected: frame
            ? selectedFrames.find((i) => i.id == frame.id) != null
            : false,
          isDivergenceSelected: modifiedFrame
            ? selectedFrames.find((i) => i.id == modifiedFrame.id) != null
            : false,
        });
      }
